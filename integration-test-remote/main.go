...
func getTest() (err error) {
	req, _ := http.NewRequest(http.MethodGet, brokerBaseURL+"/producers/", nil)
	...
	log.Println("Response", resp, brokerBaseURL)
...
func testDLQFlow() {
	...
	if waitTimeout(wg, timeoutDuration) {
		log.Println("1. Timed out waiting for wait group after", timeoutDuration)
		os.Exit(5)
	} else {
		end := time.Now()
	...
	if waitTimeout(wg, timeoutDuration) {
		log.Println("2. Timed out waiting for wait group after", timeoutDuration)
		os.Exit(12)
	} else {
		end := time.Now()
